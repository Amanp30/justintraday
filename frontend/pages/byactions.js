...
function index() {
  ...
  function getdata() {
    const options = {
      method: "GET",
      url: "http://localhost:8000/api/trades/actions",
    };
    ...
    <>
      <div className="themainclass">
        ...
        <div className="announcement">
          {data?.[0]?.pnl > data?.[1]?.pnl
            ? "You are good at Buying"
            : "You are good at selling"}
          {/*  {data?.[0]?.returns >= data?.[1]?.returns
            ? data?.[0]?.returns
            : data?.[1]?.returns} */}
        </div>
        {/* this is new div */}
                ...
                {data._id && data._id === action ? (
                  <>
                    ...
                    <div className="mainkl">
                      <div>
                        Action <span className="jiclass">{data._id}</span>
                        {data.return}
                      </div>
                      <div>
                        No. of Trades
